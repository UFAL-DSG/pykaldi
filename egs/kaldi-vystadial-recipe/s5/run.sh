...
if [ ! "$(ls -A ${MFCC_DIR} 2>/dev/null)" ]; then
  ...
  for x in train $test_sets ; do
    ...
    steps/compute_cmvn_stats.sh $fake data/$x exp/make_mfcc/$x ${MFCC_DIR} || exit 1;
    if [[ ! -z "$TEST_ZERO_GRAMS" ]] ; then
...
if [ -z $monoTrainData ] ; then
    ...
    steps/train_mono.sh --nj $njobs --cmd "$train_cmd" data/train.sub data/lang exp/mono || exit 1;
fi
...
for set_name in $test_sets_ext ; do
 ...
 steps/decode.sh --scoring-opts "--min-lmw $min_lmw --max-lmw $max_lmw" \
   --config conf/decode.config --nj $njobs --cmd "$decode_cmd" \
...
for set_name in $test_sets_ext ; do
 steps/decode.sh --scoring-opts "--min-lmw $min_lmw --max-lmw $max_lmw" \
   --config conf/decode.config --iter 4 --nj $njobs --cmd "$decode_cmd" \
  exp/tri2b/graph_${set_name} data/$set_name exp/tri2b_mmi/decode_it4_$set_name
 steps/decode.sh --scoring-opts "--min-lmw $min_lmw --max-lmw $max_lmw" \
   --config conf/decode.config --iter 3 --nj $njobs --cmd "$decode_cmd" \
