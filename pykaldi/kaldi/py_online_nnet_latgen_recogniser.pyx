...
cdef extern from "onl-rec/onl-rec-nnet-latgen-recogniser.h" namespace "kaldi":
    cdef cppclass OnlineNnetLatgenRecogniser:
...
cdef class PyOnlineNnetLatgenRecogniser:

    """PyOnlineNnetLatgenRecogniser TODO more reusable pattern
    with PyOnlineLatgenRecogniser

    Python wrapper around C++ Kaldi OnlineLatgenRecogniser
    which provides on-line speech recognition interface
    """

    cdef OnlineNnetLatgenRecogniser * thisptr
    cdef long fs
    cdef int nchan, bits
    cdef utt_decoded

    def __cinit__(self):
        self.thisptr = new OnlineNnetLatgenRecogniser()
        self.utt_decoded = 0
    ...
    def setup(self, args):
        ...
        """
        args = ['PyOnlineNnetLatgenRecogniser'] + args
